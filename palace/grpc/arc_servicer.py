...
def _arc_to_proto(a) -> palace_pb2.NarrativeArc:
    return palace_pb2.NarrativeArc(
        id=a.id,
        user_id=a.user_id,
...
class ArcServicer(palace_pb2_grpc.ArcServiceServicer):
    async def SynthesizeNarratives(
        self,
        request: palace_pb2.SynthesizeNarrativesRequest,
        context: grpc.aio.ServicerContext,
    ) -> palace_pb2.SynthesizeNarrativesResponse:
        auth = current_auth()
        ...
        if mode == "sync":
            ...
            return palace_pb2.SynthesizeNarrativesResponse(
                arcs=palace_pb2.ArcsResponse(arcs=[_arc_to_proto(a) for a in arcs]),
            )
    ...
    async def GetActiveArcs(
        self, request: palace_pb2.GetActiveArcsRequest, context: grpc.aio.ServicerContext,
    ) -> palace_pb2.ArcsResponse:
        auth = current_auth()
        tenant_id = auth.resolve_tenant()
        ...
        return palace_pb2.ArcsResponse(arcs=[_arc_to_proto(a) for a in arcs])
