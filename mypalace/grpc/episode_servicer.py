...
def _episode_dict_to_proto(d: dict) -> mypalace_pb2.Episode:
    return mypalace_pb2.Episode(
        id=str(d.get("id", "")),
...
class EpisodeServicer(mypalace_pb2_grpc.EpisodeServiceServicer):
    async def ReflectSession(
        self, request: mypalace_pb2.ReflectSessionRequest, context: grpc.aio.ServicerContext,
    ) -> mypalace_pb2.ReflectSessionResponse:
        auth = current_auth()
        ...
        if mode == "sync":
            ...
            return mypalace_pb2.ReflectSessionResponse(
                episodes=mypalace_pb2.EpisodesResponse(
    ...
    async def SearchEpisodes(
        self, request: mypalace_pb2.SearchEpisodesRequest, context: grpc.aio.ServicerContext,
    ) -> mypalace_pb2.EpisodesResponse:
        auth = current_auth()
        tenant_id = auth.resolve_tenant()
        ...
        return mypalace_pb2.EpisodesResponse(
            episodes=[_episode_dict_to_proto(r) for r in results],
        )

    async def GetRecentEpisodes(
        self, request: mypalace_pb2.GetRecentEpisodesRequest, context: grpc.aio.ServicerContext,
    ) -> mypalace_pb2.EpisodesResponse:
        auth = current_auth()
        tenant_id = auth.resolve_tenant()
